(() => {
  ...
  const state = { screen: 'home', participant: null, game: null, questions: [], answers: [], selected: null, index: 0, started: 0, record: null, saved: false, backedUp: false, filter: 'all', order: 'rank', search: '', sound: false };
  ...
  function homeScreen() {
    ...
    return `<div class="home-page">
      <section class="hero" data-action="start">
        <div class="hero-copy"><span class="eyebrow hero-badge"><span class="tiny-square"></span> NEW PLAYER WANTED!</span>
          ...
          <button class="button primary start-button" data-action="start">${pixel('controller')} 퀴즈풀기 ${pixel('arrow')}</button>
  ...
  function registerScreen() {
    return `<section class="register-page">${back()}${stepBar(0)}<div class="page-heading centered"><span class="eyebrow">HELLO, NEW PLAYER</span><h1>플레이어를 등록해주세요<span class="accent">.</span></h1><p>명예의 전당에 남길 이름, 무엇인가요?</p></div><div class="registration-card"><div class="player-card-art">${pixel('invader')}<span>PLAYER 01</span><small>오늘의 주인공은 바로 당신!</small><div class="pixel-sparkles">+ &nbsp; · &nbsp; +</div></div><form id="registration-form"><label for="player-name">이름 <span>필수</span></label><input id="player-name" name="playerName" placeholder="예: 홍길동" required maxlength="20" autocomplete="off" value="${esc(state.participant?.name || '')}"><label for="player-department">학과 <span>필수</span></label><input id="player-department" name="department" placeholder="예: 컴퓨터공학과" required maxlength="40" autocomplete="off" value="${esc(state.participant?.department || '')}"><p class="privacy-note">${pixel('heart')} 기록에는 이름이 <b>홍*동</b>처럼 가려져요.<br><span>이 기기의 브라우저에 이름(가림)과 학과, 점수를 저장해요.</span></p><button class="button primary" type="submit">게임 선택하기 ${pixel('arrow')}</button></form></div><p class="under-note">NO PRESSURE. JUST PLAY. <span>잘 몰라도 괜찮아요, 즐기면 그만!</span></p></section>`;
  }
  function gamesScreen() {
    return `<section class="games-page">${back('register', '플레이어 수정')}${stepBar(1)}<div class="page-heading"><div><span class="eyebrow">CHOOSE YOUR GAME</span><h1>당신의 주종목은 무엇인가요<span class="accent">?</span></h1><p>게임을 누르면 시작! 난이도별 한 문제씩, 총 5문제가 출제돼요.</p></div><span class="player-tag">${pixel('user')} ${esc(state.participant.name)} <small>${esc(state.participant.department)}</small></span></div><div class="games-grid">${games.map((g, i) => `<button class="game-card game-${g.id} ${g.available ? '' : 'pending-game'}" data-action="choose-game" data-game="${g.id}" aria-disabled="${!g.available}" style="--game-color:${g.color}" aria-label="${g.name}${g.available ? ' 퀴즈 시작' : ' 준비 중: 난이도 3 이미지 필요'}"><div class="game-card-top"><span>0${i + 1}</span><span class="game-status">${g.available ? 'PLAY ↗' : '준비 중'}</span></div><div class="game-logo">${logo(g)}<strong class="wordmark wordmark-${g.id}">${esc(g.english)}</strong></div><div class="game-card-bottom"><strong>${g.name}</strong><span>${g.genre}</span></div></button>`).join('')}</div><div class="games-note"><span>${pixel('bolt')} 입문 → 초급 → 중급 → 상급 → 극악</span><span>이미지 문제는 준비 중 · 롤은 이미지 등록 후 오픈돼요.</span></div></section>`;
  }
  function quizScreen() {
    const q = state.questions[state.index];
    return `<section class="quiz-page">${back('home', '퀴즈 나가기')}<div class="quiz-top"><span class="quiz-game" style="color:${state.game.color}">${logo(state.game)}<b>${state.game.name}</b></span><span class="quiz-player">${esc(state.participant.name)} 님의 도전 <span class="timer" id="timer">${timeLabel((Date.now() - state.started) / 1000)}</span></span></div><div class="level-track">${levelNames.map((name, i) => `<div class="level-node ${i === state.index ? 'current' : i < state.index ? 'complete' : ''}"><span>${i < state.index ? '✓' : i + 1}</span><small>${name}</small></div>`).join('')}</div><div class="question-panel"><div class="question-meta"><span class="difficulty difficulty-${q.level}">LEVEL ${q.level} · ${levelNames[q.level - 1]}</span><span>QUESTION <b>${pad(state.index + 1)}</b> / 05</span></div><h1 class="question-title">${esc(q.question)}</h1><div class="answers" role="group" aria-label="정답 선택">${q.options.map((option, i) => `<button class="answer-option" data-action="answer" data-option="${i}" aria-pressed="false"><span class="answer-number">${pad(i + 1)}</span><span>${esc(option)}</span><span class="answer-mark" aria-hidden="true"></span></button>`).join('')}</div><div class="question-bottom"><p id="answer-hint" aria-live="polite">가장 자신 있는 답을 하나 골라주세요.</p><button class="button primary" id="submit-answer" data-action="submit-answer" disabled>${state.index === 4 ? '결과 확인하기' : '다음 문제'} ${pixel('arrow')}</button></div></div><p class="under-note">TAKE YOUR TIME. <span>시간제한은 없어요. 천천히 생각해보세요.</span></p></section>`;
  }
  function finish() {
    state.record = { id: window.crypto?.randomUUID?.() || `${Date.now()}-${Math.random().toString(36).slice(2)}`, name: C.maskName(state.participant.name), department: state.participant.department, gameId: state.game.id, correct: C.score(state.questions, state.answers), elapsed: Math.floor((Date.now() - state.started) / 1000), date: new Date().toISOString() };
  ...
  function resultScreen() {
    ...
    const titles = ['도전하는 당신이 진짜 게이머!', '첫 도전, 아주 좋아요!', '게임 센스가 보이는데요?', '오, 제법 하는데요?', '고수의 향기가 나요!', '인정합니다. 당신은 찐 고인물!'];
    return `<section class="result-page"><div class="result-banner"><span class="eyebrow">QUEST COMPLETE!</span><div class="result-trophy">${pixel('trophy')}<span class="spark-one">✦</span><span class="spark-two">+</span></div><h1>${titles[r.correct]}</h1><p>${esc(state.participant.name)} 님, <strong>${state.game.name}</strong> 퀴즈를 모두 풀었어요.</p><div class="score-boxes"><div class="score-correct"><span>맞힌 문제</span><strong>${r.correct}<small>개</small></strong><span> ${pixel('check')} CORRECT</span></div><div class="score-wrong"><span>틀린 문제</span><strong>${5 - r.correct}<small>개</small></strong><span>× &nbsp; INCORRECT</span></div><div class="score-total"><span>나의 점수</span><strong>${r.correct * 20}<small>점</small></strong><span>OUT OF 100</span></div></div><p class="save-status ${state.saved ? '' : 'save-error'}" role="status">${state.saved ? `✓ ${esc(r.name)} · ${esc(r.department)} 님의 기록이 저장되었어요.` : '기록을 저장하지 못했어요. 아래에서 다시 시도하거나 CSV로 보관해주세요.'}</p>${state.saved ? '' : '<div class="save-retry"><button class="button secondary" data-action="retry-save">저장 다시 시도</button><button class="text-button" data-action="export-result">이 결과 CSV 저장 ↓</button></div>'}</div><div class="review-heading"><div><span class="eyebrow">ANSWER REVIEW</span><h2>정답과 해설 확인하기</h2></div><span>총 5문제 · ${timeLabel(r.elapsed)}</span></div><div class="reviews">${state.questions.map((q, i) => {
      const correct = q.answer === state.answers[i];
      return `<details class="review ${correct ? 'correct' : 'incorrect'}" ${correct ? '' : 'open'}><summary><span class="review-verdict">${correct ? '✓' : '×'}</span><span class="review-summary"><small>LEVEL ${q.level} · ${levelNames[q.level - 1]}</small><strong>${esc(q.question)}</strong></span><span class="review-state">${correct ? '정답' : '오답'}</span><span class="review-chevron">⌄</span></summary><div class="review-body"><div class="review-answer"><span>내가 고른 답</span><b class="${correct ? 'green-text' : 'red-text'}">${esc(q.options[state.answers[i]])}</b></div>${correct ? '' : `<div class="review-answer"><span>정답</span><b class="green-text">${esc(q.options[q.answer])}</b></div>`}<p>${esc(q.explanation)}</p></div></details>`;
  ...
  function recordsScreen() {
    ...
    return `<section class="records-page">${back()}<div class="page-heading"><div><span class="eyebrow">THE HALL OF FAME</span><h1>우리 부스의 명예의 전당<span class="accent">.</span></h1><p>게임에 진심인 사람들의 도전 기록을 만나보세요.</p></div><div class="records-mascot">${pixel('trophy')}</div></div><div class="record-stats"><div><span>함께한 도전</span><strong>${count}<small>회</small></strong></div><div><span>만점 플레이어의 도전</span><strong>${rows?.filter(r => r.correct === 5).length || 0}<small>회</small></strong></div><div><span>최고 기록</span><strong>${count ? Math.max(...rows.map(r => r.correct)) : '—'}<small>/ 5</small></strong></div><div class="stats-message">${pixel('invader')}<p>다음 기록의 주인공은<br><b>바로 당신!</b></p></div></div><div class="records-toolbar"><div class="records-tabs"><button class="${state.order === 'rank' ? 'active' : ''}" data-action="sort" data-order="rank">순위순</button><button class="${state.order === 'latest' ? 'active' : ''}" data-action="sort" data-order="latest">최신순</button></div><div class="record-filters"><label class="sr-only" for="game-filter">게임 필터</label><select id="game-filter"><option value="all">모든 게임</option>${games.map(g => `<option value="${g.id}" ${state.filter === g.id ? 'selected' : ''}>${g.name}</option>`).join('')}</select><label class="sr-only" for="record-search">이름 또는 학과 검색</label><input id="record-search" placeholder="이름 · 학과 검색" maxlength="40" value="${esc(state.search)}"></div><button class="text-button csv-button" data-action="export" ${!count ? 'disabled' : ''}>전체 기록 CSV ↓</button></div><div class="table-wrap"><table class="records-table"><thead><tr><th scope="col">순위</th><th scope="col">플레이어</th><th scope="col">학과</th><th scope="col">게임</th><th scope="col">맞힌 개수</th><th scope="col">참여 일시</th></tr></thead><tbody id="records-body"></tbody></table><div id="records-empty"></div></div><div class="records-footnote"><span>같은 정답 수는 공동 순위로 표시해요.</span><span>이 기기·브라우저에 보관된 기록이에요.</span></div><div class="records-bottom"><span class="eyebrow">EVERY PLAYER HAS A STORY.</span><button class="button primary" data-action="start">나도 도전하기 ${pixel('arrow')}</button></div></section>`;
  }
  function updateRecordsTable() {
    const rows = records();
    const filtered = (rows || []).filter(r => (state.filter === 'all' || r.gameId === state.filter));
    ...
    document.querySelector('#records-body').innerHTML = ranked.map(r => `<tr><td><span class="rank-number ${r.rank <= 3 ? 'top-rank rank-' + r.rank : ''}">${r.rank <= 3 ? pixel('trophy') : ''}${pad(r.rank)}</span></td><td><b>${esc(r.name)}</b></td><td>${esc(r.department)}</td><td><span class="table-game" style="--game-color:${gameById(r.gameId)?.color || '#888'}"><i></i>${esc(gameById(r.gameId)?.name || r.gameId)}</span></td><td><span class="record-score">${r.correct}<small> / 5</small>${r.correct === 5 ? '<span class="perfect-badge">PERFECT</span>' : ''}</span></td><td class="record-date">${esc(new Intl.DateTimeFormat('ko-KR', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date(r.date)))}</td></tr>`).join('');
    document.querySelector('#records-empty').innerHTML = ranked.length ? '' : `<div class="empty-state">${pixel(rows === null ? 'bolt' : 'trophy')}<h2>${rows === null ? '기록을 불러올 수 없어요' : state.search || state.filter !== 'all' ? '조건에 맞는 기록이 없어요' : '첫 번째 기록을 기다리고 있어요'}</h2><p>${rows === null ? '브라우저의 저장소 사용 설정을 확인해주세요.' : '퀴즈에 도전하고 나만의 기록을 남겨보세요.'}</p>${rows?.length ? '' : '<button class="button secondary" data-action="start">첫 도전 시작하기 ↗</button>'}</div>`;
  }
  function downloadCSV(rows) {
    const header = ['순위', '이름(가림)', '학과', '게임', '맞힌 개수', '틀린 개수', '점수', '소요 시간(초)', '참여 일시'];
    const lines = C.rankRecords(rows).map(r => [r.rank, r.name, r.department, gameById(r.gameId)?.name || r.gameId, r.correct, 5 - r.correct, r.correct * 20, r.elapsed, new Date(r.date).toLocaleString('ko-KR')]);
  ...
  document.addEventListener('click', event => {
    ...
    if (action === 'start') go('register');
    if (action === 'choose-game' && state.screen === 'games') {
      const game = gameById(button.dataset.game);
      if (!game.available) { toast('리그 오브 레전드는 난이도 3 스킬 이미지 문제를 준비하고 있어요.'); return; }
      state.game = game; state.questions = C.pickQuestions(game); state.answers = []; state.selected = null; state.index = 0; state.started = Date.now(); state.record = null; state.saved = false; state.backedUp = false; navigate('quiz');
    }
    if (action === 'answer' && state.screen === 'quiz') {
    ...
    if (action === 'submit-answer' && state.screen === 'quiz' && state.selected !== null) {
      state.answers.push(state.selected); state.selected = null;
      if (state.index === 4) finish(); else { state.index++; navigate('quiz'); }
    }
    ...
    if (action === 'export') { const rows = records(); if (rows) downloadCSV(rows); else toast('기록을 읽지 못했어요.'); }
