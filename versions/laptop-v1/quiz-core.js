(function (scope) {
  ...
  function pickQuestions(game, random = Math.random) {
    return game.levels.map((level, i) => {
      ...
      if (!pool.length) throw new Error(`난이도 ${i + 1} 문제 준비 중`);
  ...
  function validRecord(r) {
    return r && typeof r.id === 'string' && typeof r.name === 'string' && r.name.length <= 30 && typeof r.department === 'string' && r.department.length <= 60 && typeof r.gameId === 'string' && Number.isInteger(r.correct) && r.correct >= 0 && r.correct <= 5 && Number.isFinite(r.elapsed) && r.elapsed >= 0 && Number.isFinite(Date.parse(r.date));
  }
  ...
  const api = { KEY, maskName, pickQuestions, score, validRecord, readRecords, saveRecord, rankRecords, csvCell };
