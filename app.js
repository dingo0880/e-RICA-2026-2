(() => {
  ...
  const state = { screen: 'home', mode: 'quick', recordMode: 'quick', participant: null, game: null, questions: [], answers: [], selected: null, index: 0, started: 0, record: null, saved: false, backedUp: false, filter: 'all', order: 'rank', search: '', sound: false };
    ...
    return `<div class="home-page">
      <section class="hero">
        <div class="hero-copy"><span class="eyebrow hero-badge"><span class="tiny-square"></span> NEW PLAYER WANTED!</span>
          ...
          <div class="mode-start-buttons"><button class="button primary start-button" data-action="start" data-mode="quick">${pixel('controller')} 일반 모드 <small>5문제</small> ${pixel('arrow')}</button><button class="button secondary record-mode-button" data-action="start" data-mode="record">${pixel('trophy')} 기록 모드 <small>25문제</small></button></div>
  ...
  function registerScreen() {
    return `<section class="register-page">${back()}${stepBar(0)}<div class="page-heading centered"><span class="eyebrow">${state.mode === 'record' ? 'RECORD CHALLENGE · 25 QUESTIONS' : 'QUICK PLAY · 5 QUESTIONS'}</span><h1>플레이어를 등록해주세요<span class="accent">.</span></h1><p>${modeLabel(state.mode)}에 참가할 이름과 학과를 입력해주세요.</p></div><div class="registration-card"><div class="player-card-art">${pixel('invader')}<span>PLAYER 01</span><small>${state.mode === 'record' ? '25문제 기록에 도전!' : '오늘의 주인공은 바로 당신!'}</small><div class="pixel-sparkles">+ &nbsp; · &nbsp; +</div></div><form id="registration-form"><label for="player-name">이름 <span>필수</span></label><input id="player-name" name="playerName" placeholder="예: 홍길동" required maxlength="20" autocomplete="off" value="${esc(state.participant?.name || '')}"><label for="player-department">학과 <span>필수</span></label><input id="player-department" name="department" placeholder="예: 컴퓨터공학과" required maxlength="40" autocomplete="off" value="${esc(state.participant?.department || '')}"><p class="privacy-note">${pixel('heart')} 기록에는 이름이 <b>홍*동</b>처럼 가려져요.<br><span>이 기기의 브라우저에 이름(가림)과 학과, 점수를 저장해요.</span></p><button class="button primary" type="submit">게임 선택하기 ${pixel('arrow')}</button></form></div><p class="under-note">${state.mode === 'record' ? 'ALL 25 QUESTIONS. ONE RECORD.' : 'NO PRESSURE. JUST PLAY.'} <span>${state.mode === 'record' ? '한 게임의 모든 문제를 풀어요.' : '잘 몰라도 괜찮아요, 즐기면 그만!'}</span></p></section>`;
  }
  function gamesScreen() {
    const description = state.mode === 'record' ? '한 게임의 25문제 전체를 풀고 기록 모드 순위에 도전해요.' : '난이도별 한 문제씩 무작위로, 총 5문제가 출제돼요.';
    return `<section class="games-page">${back('register', '플레이어 수정')}${stepBar(1)}<div class="page-heading"><div><span class="eyebrow">${state.mode === 'record' ? 'RECORD MODE · ALL 25' : 'QUICK MODE · RANDOM 5'}</span><h1>당신의 주종목은 무엇인가요<span class="accent">?</span></h1><p>${description}</p></div><span class="player-tag">${pixel('user')} ${esc(state.participant.name)} <small>${esc(state.participant.department)}</small></span></div><div class="games-grid">${games.map((g, i) => `<button class="game-card game-${g.id}" data-action="choose-game" data-game="${g.id}" style="--game-color:${g.color}" aria-label="${g.name} ${modeLabel(state.mode)} 시작"><div class="game-card-top"><span>0${i + 1}</span><span class="game-status">${state.mode === 'record' ? '25 Q ↗' : '5 Q ↗'}</span></div><div class="game-logo">${logo(g)}<strong class="wordmark wordmark-${g.id}">${esc(g.english)}</strong></div><div class="game-card-bottom"><strong>${g.name}</strong><span>${g.genre}</span></div></button>`).join('')}</div><div class="games-note"><span>${pixel('bolt')} 입문 → 초급 → 중급 → 상급 → 극악</span><span>${modeLabel(state.mode)} · ${state.mode === 'record' ? '전체 25문제' : '랜덤 5문제'}</span></div></section>`;
  }
  function quizScreen() {
